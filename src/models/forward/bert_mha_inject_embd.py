...
from src.models.helpers.bert import get_shape_list,reshape_from_matrix,reshape_to_matrix, attention_layer,attention_output_layer
...
def forward_propagation(input_dict, filter_size, filter_number, classes, encoder, sentence_sizes=None, affinity=None, hidden_layer=0,
    ...
    Attention injection into BERT (AiBERT)
    ...
    if print_dim:
        print('---')
        print('Model: BERT multi-head attention embd inject')
        print('---')
    ...
    def embd_injection(bert_layer, W_T, D_T1=None, D_T2=None, embd_layer=True, scaling_factor=None, scaling_vector=None, gating=False):
        '''
        Injects embeddings by projecting them to BERT dim and adding them to existing embeddings
        ...
        with tf.name_scope(scope + '/injection/'):
            ...
            # attention between embds and projected layer
            with tf.name_scope('multi_head_attention_injection'):
                embd_dim = injected.get_shape().as_list() # (batch, words, embd_dim)
                projected_layer_dims = bert_layer_3D.get_shape().as_list() # (batch, words, bert_hidden)
                assert embd_dim[0]==projected_layer_dims[0]
                # assert embd_dims[1]==projected_layer_dims[1]
                # hidden as query, injected as key and value
                attention_heads = attention_layer(bert_layer_3D,injected,num_attention_heads=12,size_per_head=int(projected_layer_dims[2]/12))
                maybe_print([attention_heads], ['attention heads'], print_dim) # (batch, words, heads*head_size)
            with tf.name_scope('multi_head_attention_ffn'):
                attention_output = tf.concat([attention_heads], axis=-1)
                maybe_print([attention_output], ['attention output'], print_dim) # (batch, words, heads*head_size)
                combined = attention_output_layer(bert_layer_3D,attention_output)
